...
if [ "$OS" = "Linux" ]; then
    ...
    sudo pacman -Syu --color=always --noconfirm || fmt_warn "System upgrade encountered issues."
    sudo pacman -S --needed --color=always --noconfirm zsh git curl wget neovim nano eza bat zoxide fzf ripgrep \
      base-devel unzip zip tar xsel github-cli openssh \
    ...
    # Show package list and ask for confirmation
    fmt_info "The following packages will be installed:"
    echo "  zsh git curl wget neovim nano eza bat zoxide fzf ripgrep"
    echo "  unzip zip sevenzip gh python nodejs gcc cmake make"
    echo "  btop htop tmux tree jq yazi fd poppler imagemagick cmatrix"
    echo "  asciiquarium fastfetch lazygit yarn libnotify tldr opencode"
    read -p "Continue? [y/N]: " confirm_pkgs
    confirm_pkgs=${confirm_pkgs:-n}
    if [[ "${confirm_pkgs,,}" != "y" ]]; then
        fmt_warn "Package installation cancelled."
        exit 0
    fi
    ...
    brew install zsh git curl wget neovim nano eza bat zoxide fzf ripgrep \
      ...
      asciiquarium fastfetch lazygit yarn libnotify tldr

    # Install opencode (AI coding agent)
    brew install anomalyco/tap/opencode

    # Ask about setup options
    read -p "Setup zshrc? [y/N]: " setup_zshrc
    setup_zshrc=${setup_zshrc:-n}
    read -p "Setup git? [y/N]: " setup_git
    setup_git=${setup_git:-n}
    read -p "Setup neovim? [y/N]: " setup_neovim
    setup_neovim=${setup_neovim:-n}
fi
...
# 5. Change default shell to zsh (Linux only - macOS already has zsh by default)
if [ "$OS" = "Linux" ]; then
...
# 6. Install uv (Python Package Manager) - Linux always, macOS only if setting up zshrc
if [ "$OS" = "Linux" ] || [[ "${setup_zshrc,,}" == "y" ]]; then
...
# Install Zinit (zsh plugin manager) - Linux always, macOS only if setting up zshrc
if [ "$OS" = "Linux" ] || [[ "${setup_zshrc,,}" == "y" ]]; then
...
# 7. Download custom configurations (Linux always, macOS based on choice)
if [ "$OS" = "Linux" ] || [[ "${setup_zshrc,,}" == "y" ]]; then
...
# 8. Install Neovim Config (Linux always, macOS based on choice)
if [ "$OS" = "Linux" ] || [[ "${setup_neovim,,}" == "y" ]]; then
...
# 9. Git and GitHub Setup (Linux always, macOS based on choice)
if [ "$OS" = "Linux" ] || [[ "${setup_git,,}" == "y" ]]; then
    ...
    if [ "$OS" = "Linux" ]; then
        if ! command -v xdg-open >/dev/null 2>&1; then
            ...
            sudo pacman -S --needed --color=always --noconfirm xdg-utils >/dev/null
        fi
    fi
    ...
    if [ "$OS" = "Linux" ]; then
        if ! command -v ssh-keygen >/dev/null 2>&1; then
            ...
            sudo pacman -S --needed --color=always --noconfirm openssh
        fi
    fi
